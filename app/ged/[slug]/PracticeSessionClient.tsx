// app/GED/[slug]/PracticeSessionClient.tsx
"use client";
...
interface Question {
  ...
  section?: string; // subject id: rla, math, science, social
}
...
// Helper to convert a skill name (or any text) into a folder‑safe string
...
// Parse a skill ID: "subjectId|domainName|skillText"
function parseSkillId(skillId: string): { subject: string; domain: string; skill: string } | null {
  const parts = skillId.split("|");
  if (parts.length !== 3) return null;
  return { subject: parts[0], domain: parts[1], skill: parts[2] };
}

// Fetch one skill’s data from the public folder
// Expected structure: public/3-a-ged/{subject}/{domainFolder}/{skillFolder}/questions.json
async function fetchSkillData(skillId: string): Promise<PracticeData | null> {
  const parsed = parseSkillId(skillId);
  if (!parsed) return null;
  const { subject, domain, skill } = parsed;

  const domainFolder = toFolderName(domain);
  const skillFolder = toFolderName(skill);
  const jsonUrl = `/3-a-ged/${subject}/${domainFolder}/${skillFolder}/questions.json`;

  try {
    const res = await fetch(jsonUrl);
    if (!res.ok) return null;
    const data = await res.json();
    // Add section info (subject id) to each question to enable calculator for math
    if (data.questions) {
      data.questions = data.questions.map((q: Question) => ({ ...q, section: subject }));
    }
...
async function aggregateQuestions(skillIds: string[]): Promise<PracticeData> {
  ...
  for (const skillId of skillIds) {
    const skillData = await fetchSkillData(skillId);
...
export function PracticeSessionClient({ skillIds }: { skillIds: string[] }) {
  ...
  const transformHtml = useCallback(
    (html: string, highlight?: string): string => {
      if (!html) return "";
      // Rewrite image src paths – images are stored in `public/3-a-ged/images/`
      let transformed = html.replace(
        /src="images\/([^"]+)"/g,
        (_, fileName) => `/3-a-ged/images/${fileName}`
      );
  ...
  const handleBackToTopics = () => {
    router.push("/ged/Practice-Questions");
  };
        ...
        <div className="max-w-7xl mx-auto px-6 py-12 text-center">
          <div className="border border-[#E2E8F0] rounded-2xl p-12 bg-white shadow-sm">
            <p className="text-[#718096] text-lg">
              No questions found for the selected skills. Please check that the JSON files exist under /public/3-a-ged/.
            </p>
            <button
