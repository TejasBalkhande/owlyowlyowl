// app/act/ActClient.tsx (Client Component)
"use client";
...
export default function ActClient({ username, isLoggedIn, dashboardStats }: ActClientProps) {
      ...
      <div className="bg-[bg-gray-50] w-full overflow-visible">
        <div className="max-w-[1360px] mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
            ...
            {/* RIGHT COLUMN – Dashboard Sidebar (always visible) */}
            <aside className="lg:col-span-1 space-y-6">
              ...
              <div className="bg-white rounded-xl shadow-md border border-[#E2E8F0] p-5">
                ...
                <p className="text-xs text-[#718096] mt-1">
                  {isLoggedIn && username ? `Welcome back, ${username}!` : "Welcome back, Guest!"}
                </p>
              </div>

              {/* Always show the dashboard cards, but display zeros/empty states */}
              <div className="space-y-6">
                {/* Progress Overview Card */}
                <div className="bg-white rounded-xl shadow-md border border-[#E2E8F0] p-5">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="font-semibold text-[#2D3748]">Progress</h3>
                    <div className="flex items-center gap-2">
                    <span className="text-xs bg-green-100 text-green-700 px-1.5 py-1 rounded-full">
                        {dashboardStats.totalQuestions > 0 ? "On track" : "Not started"}
                    </span>
                    </div>
                  </div>
                ...
                {/* Optional: Sign-in prompt for non-logged-in users */}
                {!isLoggedIn && (
                  <div className="bg-white rounded-xl shadow-md border border-[#E2E8F0] p-5 text-center">
                    <p className="text-sm text-[#4A5568] mb-3">
                      Create a free account to save your progress and get personalized recommendations.
                    </p>
                    <Link href="/account" className="inline-block px-4 py-2 bg-[#1E4A76] text-white rounded-lg text-sm font-medium hover:bg-[#163A5E] transition">
                  ...
                  <div className="flex items-end justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs text-[#718096]">
                        {dashboardStats.predictedScore === 0
                          ? "Take a practice test to see your predicted score"
                    ...
                    <ul className="space-y-2">
                      {dashboardStats.weakAreas.map((area, i) => (
                        <li key={i} className="flex items-center justify-between text-sm">
                          ...
                          <span className="text-xs bg-red-100 text-red-600 px-2 py-0.5 rounded-full">weak</span>
                        </li>
                      ))}
                    </ul>
                  ) : (
                    <p className="text-sm text-[#718096]">
                      {isLoggedIn ? "No weak areas yet – keep practicing!" : "Complete practice topics to see weak areas."}
                    </p>
                  )}
                ...
                <div className="bg-white rounded-xl shadow-md border border-[#E2E8F0] p-5">
                  ...
                  {dashboardStats.recentActivity.length > 0 ? (
                    ...
                    <p className="text-sm text-[#718096]">
                      {isLoggedIn ? "No activity yet – start practicing!" : "Sign in to track your recent practice."}
                    </p>
                  )}
          ...
          {/* MOVED SECTIONS – full width and centered (unchanged) */}
          <div className="w-full flex justify-center my-5">
            <div className="w-full max-w-5xl h-px bg-gradient-to-r from-transparent via-[#E2E8F0] to-transparent"></div>
          </div>
